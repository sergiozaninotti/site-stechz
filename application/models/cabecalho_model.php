...
class Cabecalho_model extends CI_Model
{
    ...
    public function get_data($id, $select=NULL)
    {
        ...
        if (!empty($select)) { 
            ...
            $this->db->from("cabecalho");
            $this->db->where("cabecalho_id",$id);
            return $this->db->get();
    ...
    {
        $this->db->insert("cabecalho",$data);
    }

    public function update($id, $data)
    {
        $this->db->where("cabecalho_id",$id);
        $this->db->update("cabecalho",$data);
    }

    public function delete($id)
    {
        $this->db->where("cabecalho_id",$id);
        $this->db->delete("cabecalho");
    }
    ...
    public function is_duplicated($field,$value,$id = NULL){
        ...
        return $this->db->get()->num_rows() > 0; // retorna todos os dados se for encontrado alguma coisa na linha da busca
                            ...
                            // ordem fica assim, [0] pega o cabecalho_name, [1] nada, [2] crouse_description
    var $column_search = array("cabecalho_name","cabecalho_description"); // variavel global da classe que recebe as colunas onde serão feito as buscas pelo datatable
    var $column_order = array("cabecalho_name","","cabecalho_duration"); // variavel recebe os campos do db que pegara a ordem de duração, seguindo a sequencia que esta no banco de dados

    private function _get_data_table(){ // função privada que é utilizada somente por outras funções aqui no model
        ...
        $this->db->from("cabecalho"); // seleciona da onde vai pegar a informação

        if($search){
            $first = TRUE; // variavel que irá verificar se é a primeira vez q faz a busca no db
            
            foreach ($this->column_search as $field) {// se existir o search, então pega a variavel coluna que vem da classe, pega se o valor passado tiver dentro do cabecalho_name ou cabecalho_description
                if($first){
        ...
        if(isset($order)){ // veririca se o order foi setado, ou seja , se o usuario clicou
            $this->db->order_by($this->column_order[$order_column], $order_dir); // pega a ordem pela coluna column_order e passa a posição $order_column que é o cabecalho_name
    ...
    public function records_filtered(){ // lista quantos dados tem no db e quantos esta mostrando naquela busca
    ...
    public function records_total(){ // lista quantos dados tem no db e quantos esta mostrando naquela busca
        $this->db->from("cabecalho"); // seleciona o db e a tabela
